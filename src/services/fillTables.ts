...
function createRowSub(
  ...
  for (let i = 0; i < Math.min(20, totalSubnets); i++) {
      ...
      if (ipArray[indexFinded < 3 ? indexFinded : 3] >= 255) break
    
  }
  return rows
}

function createRowHost(
          ipArray: number[], 
          totalSubnets: number,
          currOctate: number,
          subnet: number
...
export default function fillTables(ip: IPv4, totalSubnets: number, type: 'sub' | 'host', subnets:number) {
  ...
  return type === 'sub' 
    ? createRowSub(manipulableIP,adjustedSubnets,indexFinded)
    : createRowHost(manipulableIP, totalSubnets,2,subnets)
}
